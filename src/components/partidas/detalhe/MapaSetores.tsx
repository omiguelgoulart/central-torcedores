...
const setores = [
  { id: "jk", nome: "Arquibancada JK (Juscelino)", preco: 50, disponibilidade: 150, box: { left: 18, top: 17, width: 60, height: 12 } }, // topo
...
export function MapaSetores({ partidaId }: { partidaId: string }) {
  const [selecionado, setSelecionado] = useState<(typeof setores)[number] | null>(null);
        ...
        <CardContent className="grid md:grid-cols-[1.4fr_1fr] gap-6 items-start">
          ...
          <div
            className={`relative rounded-xl overflow-hidden border bg-muted/40 mx-auto md:mx-0 transition-all duration-300 ${
              selecionado ? "md:ml-0" : "md:col-span-2 md:flex md:justify-center"
            }`}
          >
            <div
